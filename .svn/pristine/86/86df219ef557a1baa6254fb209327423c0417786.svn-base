...
public class OpenFileDialog {
    ...
    public static String sParent = "上一层";
    public static String sFolder = ".";
    public static String sEmpty = "";
    private static String sOnErrorMsg = "No rights to access!";
    private static String wenjianjia = sRoot;
    ...
    public Dialog createDialog(int id, final Context context, String title, CallbackBundle callback, String suffix, Map<String, Integer> images) {
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        builder.setView(new FileSelectView(context, id, callback, suffix, images));
        builder.setPositiveButton("选择该文件夹", new OnClickListener() {

            @Override
            public void onClick(DialogInterface dialog, int which) {
                Log.e("result","pt:"+wenjianjia+";\nroot:"+sRoot);
                Log.e("result","path_:"+Environment.getExternalStorageDirectory().toString());///storage/emulated/0
                if (wenjianjia.equals("")||wenjianjia==null||wenjianjia.equals(sRoot)){
                    ToastUtil.showBottomShort(context,"请选择路径");
                    return;
                }
                // 设置回调的返回值
                Bundle bundle = new Bundle();
                bundle.putString("path", wenjianjia);
                // 调用事先设置的回调函数
    ...
    static class FileSelectView extends ListView implements OnItemClickListener {
        ...
        private int refreshFileList() {
            ...
            try {
                ...
                Log.i("TAG", "files:" + files.length + path);
            } catch (Exception e) {
                files = null;
            }
            if (files == null) {
            ...
            Log.i("TAG", "path:" + path + ",sRoot:" + sRoot);
            ...
            for (File file : files) {
                if (file.isDirectory() && file.listFiles() != null) {
                    if (type==1){
                        // 添加文件夹
                        if (file.getName().equals("sdcard1")){
                            Map<String, Object> map = new HashMap<String, Object>();
                            map.put("name", "SD卡");
        ...
        public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
            ...
            Log.e("result","pt:"+pt+"\nf1:"+pt);
            if (pt.equals("/storage/sdcard1")){
                if (!pt.equals(Environment.getExternalStorageDirectory().toString())){
                    ToastUtil.showBottomShort(getContext(),"请选择手机储存位置为sd卡，否则无法在sd卡中写入内容");
                    return;
                }
            }
            wenjianjia = pt;
